...
dat_seds_gubernatorial_incumbent = read_csv("cleaned_data/merged_datasets/seds_gubernatorial_inc_data.csv") %>% 
    filter(year != 2024)

  summary(dat_seds_gubernatorial_incumbent$incumbent_vote_pct)

# ---------- Main Model ----------

dat_fd_gubernatorial_incumbent = dat_seds_gubernatorial_incumbent %>%
  ...
  mutate(
    ...
    s_d_gas = d_gas^2, 
    d_log_gas = log(gas_price) - lag(log(gas_price)),
  ...
  filter(!is.na(d_gas), !is.na(d_inc_vote)) %>% 
    filter(!is.na(d_gas), !is.na(d_inc_vote)) %>% 
    select(state,
    year, county_fips, d_inc_vote, d_gas, s_d_gas, d_log_gas, d_unemp,
...
# Check how many obs you get
cat("FD observations:", nrow(dat_fd_gubernatorial_incumbent), "\n")

dat_fd_gubernatorial_incumbent %>%
  group_by(year) %>%
...
write_csv(dat_fd_gubernatorial_incumbent, "cleaned_data/merged_datasets/fd_g_inc.csv")


# ---------- Lag Placebo Data) ----------
dat_fd_g_with_lag = dat_fd_gubernatorial_incumbent %>%
  arrange(county_fips, year) %>%
  group_by(county_fips) %>%
  mutate(
    d_gas_lag = lag(d_gas),
    s_d_gas_lag = lag(s_d_gas),
    d_percent_pt_lag = lag(d_percent_pt)
  ) %>%
  filter(!is.na(d_inc_vote), !is.na(d_gas_lag)) %>% 
  filter(year != 2012) # get rid of the false d_gas_future 


write_csv(dat_fd_g_with_lag, "cleaned_data/merged_datasets/fd_g_inc_lag_placebo.csv")

# ---------- Future Placebo Data  (->2016) ----------

dat_fd_g_with_future = dat_fd_gubernatorial_incumbent %>%
  arrange(county_fips, year) %>%
  group_by(county_fips) %>%
  mutate(
    d_gas_future = lead(d_gas),
    s_d_gas_future = lag(s_d_gas),
...
write_csv(dat_fd_g_with_future, "cleaned_data/merged_datasets/fd_g_inc_future_placebo.csv")
